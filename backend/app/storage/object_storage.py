...
class MinIOStorage(ObjectStorage):
    ...
    def __init__(
        ...
        # Ensure bucket exists. Multiple services (backend, worker) can race
        # to create it on a fresh volume -- the loser's make_bucket() raises
        # BucketAlreadyOwnedByYou/BucketAlreadyExists, which just means the
        # bucket is now there, not that init failed.
    ...
    def put_bytes(self, key: str, data: bytes, content_type: str = "application/octet-stream") -> str:
        ...
        # Write-through the local cache so a subsequent get_local_path() for
        # this key doesn't need a round-trip to MinIO.
...
@lru_cache
def get_storage_backend() -> ObjectStorage:
